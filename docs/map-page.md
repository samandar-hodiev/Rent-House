...
- **Map controls** (`components/MapControls.jsx`): Layers, My Location, Zoom In, Zoom Out — plain React buttons, not the map provider's own controls (the map is created with `controls: []`). They call `mapControllerRef.current.zoomIn()/.zoomOut()` and the existing `handleLocateRequest`. The component is rendered twice, once per breakpoint, so each instance owns its layers-popover state:
  - **Desktop/tablet (`sm:` and up):** a single **horizontal row** in the map's bottom-right corner, in its own compact glass container, offset up (`pb-10`) so it sits above the Yandex attribution/logo strip rather than covering it. Its layers menu opens **upward** (`menuPlacement="top"`) so it never runs off the bottom edge.
  - **Mobile:** a horizontal row on the second line of the *same* top glass bar as the filters (the bar switches to `flex-col`), right-aligned, sharing that line with the result-count pill. There is no second floating control container on mobile.
- **Single-row filter row:** the filter button stays pinned at the start and the active chips scroll horizontally beside it — they never wrap. `FilterBar`'s `singleRow` prop puts **only the chips** (plus "Clear all") in an `overflow-x-auto` container while the button stays `shrink-0` outside it; chips are `shrink-0 whitespace-nowrap`. Exactly one scroll container is involved: an `overflow-x-auto` on an ancestor as well would let the inner row shrink and clip its chips with nothing left to scroll.
- **Result count placement:** the count pill is a status indicator, not a filter chip, so it is never inside the chip scroll area. On desktop it sits next to the filter row; on mobile it moves to the second line beside the map controls, which frees roughly twice the width for chips without adding any height.
